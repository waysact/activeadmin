...
module ActiveAdmin
  module Views
    ...
    class PaginatedCollection < ActiveAdmin::Component
      ...
      # Adding div with pagination control
      def build(collection, options = {})
        @collection = collection

        if ['Pledge', 'AbsenceReason'].include?(collection.first.class.name)
          @download_links = false
          div id: "per_page" do
            links = ['200', '500', '1000'].collect do |per_page|
              link_to per_page.to_s, per_page_admin_pledges_path(per_page: per_page), remote: true, class: classes_for_per_page(per_page)
            end
            text_node [links, 'per page'].flatten.join('&nbsp;').html_safe
          end
        end
      ...
      def classes_for_per_page(per_page)
        classes = ['pagination_element']
        classes << 'selected' if session[:per_page] == per_page
        classes.join(' ')
      end
      ...
      # backporting fix from https://github.com/gregbell/active_admin/issues/751
      # TODO remove this when we upgrade to Active Admin 0.4.0
      def page_entries_info(options = {})
        if options[:entry_name]
          entry_name = options[:entry_name]
